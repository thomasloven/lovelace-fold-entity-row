...
class FoldEntityRow extends LitElement {
  ...
  setConfig(config) {
    ...
    if (typeof head === "string") head = { entity: head };
    ...
    else if (items === undefined) throw new Error("No entities specified.");
  ...
  hasMoreInfo(config) {
    const entity =
      config.entity || (typeof config === "string" ? config : null);
