...
async function findParentCard(node, step = 0) {
    ...
    if (node.parentElement)
        return findParentCard(node.parentElement);
    else if (node.parentNode)
        return findParentCard(node.parentNode);
    if (node.host)
        return findParentCard(node.host);
    return false;
}
class FoldEntityRow extends LitElement {
    ...
    setConfig(config) {
        ...
        if (head.entity && items === undefined) {
            items = (_b = (_a = hass().states[head.entity]) === null || _a === void 0 ? void 0 : _a.attributes) === null || _b === void 0 ? void 0 : _b.entity_id;
        }
        if (items === undefined) {
    ...
    async applyStyle(root, config, head = false) {
        ...
        customElements
            .get("card-mod")
            .applyToElement(root, "row", config.card_mod ? config.card_mod.style : config.style, { config });
    }
    toggle(ev) {
        if (ev)
            ev.stopPropagation();
    ...
    update(changedProperties) {
        ...
        if (changedProperties.has("open")) {
            const el = this.shadowRoot.querySelector("#items");
            if (this.open)
    ...
    firstUpdated() {
        ...
        this.shadowRoot
            .querySelector("#head")
            .addEventListener("click", (ev) => this._handleClick(ev), {
            capture: true,
        });
